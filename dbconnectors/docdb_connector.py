...
def get_collection_link(id_):
    return get_db_link() + '/colls/{0}'.format(id_)


def get_document_link(collection_id, id_):
    return get_collection_link(collection_id) + '/docs/{0}'.format(id_)
...
def create(collection_id, docs):
    with IDisposable(get_client()) as client:
        ...
        if type(docs) is dict:
            docs = [docs]
        for doc in docs:
            new_doc = client.CreateDocument(get_collection_link(collection_id), doc)
            created_ids.append(new_doc[ID])
        return created_ids


def read(collection_id, query_dicts):
    with IDisposable(get_client()) as client:
        docs = []
        if query_dicts is None:
            docs = client.ReadDocuments(get_collection_link(collection_id)).fetch_items()
        else:
            for query_dict in query_dicts:
                try:
                    # TODO: Optimize! If the only query involves an id then use ReadDocument instead
                    doc = client.QueryDocument(get_collection_link(collection_id), query_dict)
                    docs.append(doc)
...
def update(collection_id, new_docs):
    with IDisposable(get_client()) as client:
        ...
        for new_doc in new_docs:
            try:
                existing_doc = client.ReadDocument(get_document_link(collection_id, new_doc[ID]))
                existing_doc.update(new_doc)
                client.ReplaceDocument(get_document_link(collection_id, existing_doc[ID]), existing_doc)
...
def delete(collection_id, ids):
    with IDisposable(get_client()) as client:
        ...
        for id_ in ids:
            try:
                client.DeleteDocument(get_document_link(collection_id, id_))
